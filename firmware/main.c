...
static uint8_t touch_sample(uint8_t sensepin);
...
static uint8_t touch_sense(uint8_t pad) {
    uint16_t tmp=0;
    int16_t delta;
...
#define PURR_FADE 4
        ...
        val16 = pgm_read_byte(&purr[phase>>(16-WAVE_LOG2)]);
        val16 *= final_vol;
        val8 = val16 >> 8;
        val8 ^= 0x80;
    }
